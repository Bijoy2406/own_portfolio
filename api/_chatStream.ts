...
// ─── Shared chat-stream logic used by both api/chat.ts (Vercel prod) and the
// dev middleware in vite.config.ts. Keeps the two paths in lock-step so model
// selection + fallback behavior is identical locally and in production.
